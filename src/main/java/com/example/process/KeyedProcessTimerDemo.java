...
/***
 * @author: BYDylan
 * @date: 2024-09-22 12:55:58
 * @description: 按键分区处理函数 1、keyed才有 2、事件时间定时器, 通过watermark来触发的 watermark >= 注册的时间 注意： watermark = 当前最大事件时间 - 等待时间 -1ms,
 *               因为 -1ms, 所以会推迟一条数据 比如, 5s的定时器, 如果 等待=3s, watermark = 8s - 3s -1ms = 4999ms,不会触发5s的定时器 需要 watermark = 9s
 *               -3s -1ms = 5999ms , 才能去触发 5s的定时器 3、在process中获取当前watermark, 显示的是上一次的watermark
        ...
        SingleOutputStreamOperator<String> process = sensorKS.process(
            new KeyedProcessFunction<>() {
                ...
                public void processElement(WaterSensor value, Context ctx, Collector<String> out) {
                    ...
                    log.info("processElement key: {}, time: {}", currentKey, currentEventTime);
                    ...
                    log.info("process value: {}, current watermark: {}", value, currentWatermark);
                 ...
                 */
                    ...
                    public void onTimer(long timestamp, OnTimerContext ctx, Collector<String> out) throws Exception {
                        ...
                        String currentKey = ctx.getCurrentKey();
                    log.info("onTimer key: {}, time: {}", currentKey, timestamp);
                    }
                }
        );
        process.print();
