...
/***
 * @author: BYDylan
 * @date: 2024-09-22 17:33:59
 * @description: 在map算子中计算数据的个数: 算子状态中, list 与 unionlist的区别: 并行度改变时, 怎么重新分配状态 1、list状态： 轮询均分 给 新的 并行子任务 2、unionlist状态：
 *               原先的多个子任务的状态, 合并成一份完整的。 会把 完整的列表 广播给 新的并行子任务 （每人一份完整的）
 */
@Slf4j
public class OperatorListStateDemo {
    ...
    public static class MyCountMapFunction implements MapFunction<String, Long>, CheckpointedFunction {
        ...
        public void initializeState(FunctionInitializationContext context) throws Exception {
            ...
            // 3.1 从 上下文 初始化 算子状态
            state = context.getOperatorStateStore().getListState(new ListStateDescriptor<>("state", Types.LONG));
