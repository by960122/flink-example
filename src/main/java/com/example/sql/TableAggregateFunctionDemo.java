...
public class TableAggregateFunctionDemo {
    public static void main(String[] args) throws Exception {
        ...
        Schema schema = Schema.newBuilder().column("num", DataTypes.INT()).build();
        Table numTable = tableEnv.fromDataStream(numDS, schema);
        // 2.注册函数
        tableEnv.createTemporaryFunction("Top2", Top2.class);
        // 3.调用 自定义函数: 只能用 Table API
        numTable.flatAggregate(call("Top2", $("num")).as("value", "rank")).select($("value"), $("rank")).execute()
            .print();
    }
