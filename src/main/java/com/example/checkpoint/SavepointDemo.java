...
/***
 * @author: BYDylan
 * @date: 2024-09-22 17:55:40
 * @description: 保存点: 与检查点最大的区别, 就是触发的时机。检查点是由 Flink自动管理的, 定期创建, 发生故障之后自动读取进行恢复, 这是一个“自动存盘”的功能；而保存点不会自动创建,
 *               必须由用户明确地手动触发保存操作, 所以就是“手动存盘”, 通过在启动命令中配置(flink run)
 */
public class SavepointDemo {
    public static void main(String[] args) throws Exception {
        ...
        env.socketTextStream("hadoop102", 7777).uid("socket")
            ...
            }).uid("flatmap-wc").name("wc-flatmap").returns(Types.TUPLE(Types.STRING, Types.INT))
            .keyBy(value -> value.f0).sum(1).uid("sum-wc").print().uid("print-wc");
        env.execute();
    }
}
